...
namespace BehaviorDesigner.Editor.ObjectDrawers
{
    ...
    public class HotfixBtActionDrawer : ObjectDrawer
    {
        ...
        public override void OnGUI(GUIContent label)
        {
            ...
            {
                fileNameArray = null;
                var hotfixBtStringAttribute = (HotfixBtActionAttribute)attribute;
                string hotfixBtPath = Path.Combine(Application.dataPath, hotfixBtStringAttribute.hotfixBtActionPath);
                var hotfixActionCreater = new HotfixCodeGeneratePopup(hotfixBtPath);
                PopupWindow.Show(buttonRect, hotfixActionCreater);
            }
            else
            {
                value = "";
    ...
    public class HotfixCodeGeneratePopup : PopupWindowContent
    {
        public HotfixCodeGeneratePopup(string saveScriptPath) : base()
        {
            saveScriptLocation = saveScriptPath;
        }
        public string saveScriptLocation;
        const string codeTemplate = @"-- This code template is generated by HotfixBtActionDrawer.cs
...
";
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            GUILayout.Space(5);
            GUILayout.Label("请输入 Hotfix Bt 脚本名称: ");
            GUILayout.Space(2);
            createFileName = EditorGUILayout.TextField(createFileName);
            GUILayout.Space(5);
            if (GUILayout.Button("新建 Hotfix Bt 脚本", GUILayout.ExpandHeight(true)))
            {
